...
* The USE_STDEV build knob decides whether the device number recorded in the
  index is compared against the working tree. Whether that number stays the
  same for a file is a property of the filesystem holding the repository, and
  one Git reaches several filesystems, so a single answer chosen when Git was
  compiled cannot suit all of them. `core.trustStdev` says the same thing per
  repository and USE_STDEV will be removed once it has been available for long
  enough that a build relying on it has somewhere to go.

* The MMAP_PREVENTS_DELETE build knob says that a file which is currently
  mmapped cannot be deleted or replaced by rename(). That is a property of the
  filesystem holding the repository rather than of the build, and one Git
  reaches both kinds at once, so `core.mmapPreventsDelete` says it per
  repository and MMAP_PREVENTS_DELETE will be removed. NO_MMAP stays, since
  whether this Git can map a file at all is settled when it is built.

* The NO_FAST_WORKING_DIRECTORY build knob says that reading a file from the
  working directory is slower than obtaining the same contents from a pack.
  How quickly a working directory answers is a property of the filesystem
  holding it, and one Git reaches a slow filesystem and a fast one at once, so
  `core.fastWorkingDirectory` says it per repository and
  NO_FAST_WORKING_DIRECTORY will be removed.

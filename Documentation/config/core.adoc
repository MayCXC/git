...
core.trustStdev::
	If true, the device number recorded in the index is compared
	against the working tree, so that a file arriving on a
	different device counts as changed. Filesystems whose clients
	each decide the device number for themselves, network
	filesystems among them, give a different answer to each client
	and to the same client over time, and one Git can reach such a
	filesystem and an ordinary one at once. False by default, and
	ignored when `core.checkStat` is `minimal`.

core.mmapPreventsDelete::
	If true, a file that is currently mmapped cannot be deleted or
	replaced using rename(), so Git copies the contents of the
	`packed-refs` file and unmaps it rather than holding the
	mapping open. Windows behaves this way, and so do some network
	filesystems, while one Git can reach such a filesystem and an
	ordinary one at once. False by default.

core.fastWorkingDirectory::
	If true, reading a file from the working directory is at least
	as fast as obtaining the same contents from a pack, so Git
	reads the file. Where a filesystem is slow with its stat,
	open, mmap and close calls, setting this to false lets Git
	read from an already open pack instead when it does not need
	the contents in a file of their own. True by default.
...
core.checkStat::
	...
	the device number, if `core.trustStdev` is set), are

...
objectStorage:::
	Specify the object storage backend. The value is a single name: `files`
	(the default: loose objects and packfiles) or any other name, which
	selects the `git-local-<name>` object helper program that serves objects
	over a line protocol, the way `refStorage` selects a reference helper and
	an unknown URL scheme selects a `git-remote-<name>` helper.
+
Like `refStorage`, this should only be set by linkgit:git-init[1] or
linkgit:git-clone[1]; convert an existing repository with `git odb migrate
--object-storage=<name>` instead.
...
* Any other name selects the `git-local-<name>` reference helper program,
  exactly as an unknown URL scheme selects a `git-remote-<name>` helper; the
  helper then serves all reference operations over a line protocol.

Builtin format names are:
